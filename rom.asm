...
1:		clc			; enter native mode
		...
		mvn $0707		; top 64K
		mvn $0606		; bottom 64K
		mvn $0000		; vectors/handlers, now DBR=0
...
NAK=21
	...
	;
	; Upon entry 'xmaddr' holds the far address of the
	; destination buffer.
	;
	; Upon return, carry is set if the transfer was
	; successful, or clear if there was an error.
...
8:		jsr xmdrain
		...
		clc			; no more retries, fatal 
 		rts			

	; receive byte into A
	; X is the timeout in hundreths of a second
	; on return, carry is set on success, or clear on timeout
...
xmdrain:	jsr recvflush		; flush receive buffer
		ldx #100		; one second
		jsr xmrecv
		bcs >xmdrain		; if we got a char, start again
		rts
...
recvflush:	php
	...
	; carry set if data available, cleared otherwise
...
; these CRC tables must be page-aligned
...
;
; This last page is assembled at $6ff00-$6ffff but is copied to 
; $00ff00 - $00ffff, because the 65816 requires that vectors 
; and the handlers referenced in the vectors be in bank 0.
;
; irq runs in bank 0
;
; recvidle runs both in bank 0 (when it's called from irq)
; and bank 6 (when it's called from the main code)
;
; these routines could probably be optimized, and the better
; optimized they are, the more reliably we can support higher
; baud rates 
;

IRQ_LATENCY=38				; IRQ latency - see below
...
brk:		sep #>%00100000		; 8-bit accumulator
		pha
